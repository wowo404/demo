...
public class ImageUtils {
    ...
    public static String IMAGE_TYPE_TIFF = "tiff";//标签图像文件格式
...
//		mergeImage("C:/Users/liuzhsh/Desktop/6.jpg","C:/Users/liuzhsh/Desktop/blank.jpg","C:/Users/liuzhsh/Desktop/e.jpg","下载We社区（银川小松鼠公司推广）");
     ...
     * 拼接图片
     *
     * @param srcImageOne   源文件1
     * @param srcImageTwo   源文件2
     * @param destImageFile 目标文件
     * @param pressText     水印文字
     */
    public static void mergeImage(String srcImageOne, String srcImageTwo, String destImageFile, String pressText) {
        try {
            //读取第一张小图，创建它的BufferedImage
            BufferedImage buffImageOne = ImageIO.read(new File(srcImageOne));
            int type1 = buffImageOne.getType();
            int chunkWidth1 = buffImageOne.getWidth();
            int chunkHeight1 = buffImageOne.getHeight();
            //读取第二张小图，创建它的BufferedImage
            BufferedImage blankImage = ImageIO.read(new File(srcImageTwo));
            int type2 = blankImage.getType();
            int chunkWidth2 = blankImage.getWidth();
            int chunkHeight2 = blankImage.getHeight();
            BufferedImage buffImageTwo = new BufferedImage(chunkWidth2, chunkHeight2, type2);

            //给第二张图片添加文字水印
            int fontSize = 15;
            int fixX = 0;//横轴修正值
            int fixY = 15;//竖轴修正值
            Graphics2D g = buffImageTwo.createGraphics();
            g.drawImage(blankImage, 0, 0, chunkWidth2, chunkHeight2, null);
            g.setColor(Color.BLACK);
            g.setFont(new Font("黑体", Font.BOLD, fontSize));
            g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, 0.5f));

            // 在指定坐标绘制水印文字
            g.drawString(pressText, (chunkWidth2 - (getLength(pressText) * fontSize))
                    / 2 + fixX, (chunkHeight2 - fontSize) / 2 + fixY);
            g.dispose();

            //设置拼接后图的大小和类型
            BufferedImage finalImg = new BufferedImage(chunkWidth1, chunkHeight1 + chunkHeight2, type1);

            //写入图像内容，通过x和y控制放入图片的位置,xy是从左上角开始
            finalImg.createGraphics().drawImage(buffImageOne, 0, 0, null);//二维码图片
            finalImg.createGraphics().drawImage(buffImageTwo, 0, chunkHeight1, null);//blank图片

            //输出拼接后的图像
            ImageIO.write(finalImg, "jpg", new File(destImageFile));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void compressPictureByQuality(String srcFilePath, String dest, float quality) {
        ...
        // 指定写图片的方式为 jpg
        ImageWriter imgWriter = ImageIO.getImageWritersByFormatName(suffix).next();
        ...
        try {
            ...
            ColorModel colorModel = ImageIO.read(srcFile).getColorModel();
